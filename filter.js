...
document.addEventListener("DOMContentLoaded", function() {
    ...
    processButton.addEventListener("click", function() {
        ...
        if (filteredImage) {
            ...
            img.onload = function() {
                ...
                const similarityPercentage = calculateImageSimilarity(imageData, restoredImageData);
    ...
    function calculateImageSimilarity(imageData1, imageData2) {
        ...
        const similarityPercentage = 100 - (totalDifference / maxPossibleDifference * 100);
    ...
    function reverseFilter(imageData) {
        ...
        try {
            ...
            const [restoredR, restoredG, restoredB] = applyAdvancedReverseFiltering(R, G, B, width, height);
    ...
    function applyBilateralFiltering(R, G, B, width, height, radius, sigmaSpace, sigmaRange) {
        const resultR = new Array(width * height);
        const resultG = new Array(width * height);
        const resultB = new Array(width * height);
        ...
        for (let y = 1; y < height - 1; y++) {
            for (let x = 1; x < width - 1; x++) {
                for (let c = 0; c < 3; c++) {  // For R, G, B channels
                    ...
                    // Set the result
                    resultPixels[(y * width + x) * 4 + c] = Math.max(0, Math.min(255, sum));
                }
